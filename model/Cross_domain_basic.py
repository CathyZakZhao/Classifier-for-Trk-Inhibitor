...
def KNN_gridsearch(tr_scaled_x,tr_y,clus):
    warnings.filterwarnings("ignore")
    ...
    for j in range(nFolds):
        if j!=clus:
            trainIndices = tr_cv[tr_cv['km_clus']!=j].index.values.astype(int)
            testIndices = tr_cv[tr_cv['km_clus']==j].index.values.astype(int)
        else:
            continue
        myCViterator.append((trainIndices,testIndices))
    grid_cv = myCViterator 
...
def DT_gridsearch(tr_scaled_x,tr_y,clus):
    warnings.filterwarnings("ignore")
    ...
    return Grid.best_params_ ,Grid.best_estimator_
def SVC_gridsearch(tr_scaled_x,tr_y,clus):
    warnings.filterwarnings("ignore")
    grid_list = []
    ...
    Grid.fit(tr_scaled_x, tr_y)
    return Grid.best_params_ ,Grid.best_estimator_
def RF_gridsearch(tr_scaled_x,tr_y,clus):
    warnings.filterwarnings("ignore")
    criterion_list = ['gini','entropy']
    max_features_list = [None,'sqrt','log2']
    min_samples_split_list = range(2,11)
    ...
    return Grid.best_params_ ,Grid.best_estimator_
def LG_gridsearch(tr_scaled_x,tr_y,clus):
    warnings.filterwarnings("ignore")
...
for i in range(10):
    ...
    clus = i
    tr_csv = tr_csv_initial.reset_index(drop=True)
    te_csv = te_csv_initial.reset_index(drop=True)

    tr_x, tr_y, te_x, te_y, tr_cv = extract_data(tr_csv,te_csv) #descriptors_list
    tr_scaled_x, te_scaled_x = data_scale(tr_x, te_x)
###
    best_params, best_estimator = KNN_gridsearch(tr_scaled_x, tr_y,clus)
...
'''
    ...
    every_result['best_params']=best_params
     every_out_csv_resulut = r"...\everyclus_moe2d_kmeans_KNN.csv" #Modified for different models
    every_output[i] = pd.Series(every_result,dtype=np.object)
...
out_csv_resulut = "alldata_moe2d_kmeans_KNN_ocv.csv" ###Modified for different models
