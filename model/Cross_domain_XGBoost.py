...
all_csv = pd.read_csv('alldata_moe2d_pick_model_kmeans.csv')
for i in range(10):
    tr_csv_initial = all_csv[all_csv['km_clus']!=i]
    te_csv_initial = all_csv[all_csv['km_clus']==i]
    clus = i
    train_data_1  = tr_csv_initial.reset_index(drop=True)
    test_data_1 = te_csv_initial.reset_index(drop=True)
    ...
    for j in range(nFolds):
        if j!=clus:
            trainIndices = train_data_1[train_data_1['km_clus']!=j].index.values.astype(int)
            testIndices = train_data_1[train_data_1['km_clus']==j].index.values.astype(int)
        else:
            continue
        myCViterator.append((trainIndices,testIndices))
    grid_cv = myCViterator
    clf = GridSearchCV(model,parameters,scoring=grid_scorer,n_jobs=-1,cv=grid_cv)
...
plt.title('XGB_MOE_KMEANS_ROC',size=25)
plt.show()
